 ...
 * 
 * compile: gcc -Wall -O2 -ffast-math -o sell_target sell_target.c `mysql_config --include --libs` `curl-config --libs`
 */
...
int main(int argc, char * argv[]) {
  ...
  do {
    ...
    if (TM->tm_hour<strtoul(market_open,&saveptr,10) || 
      (TM->tm_hour==strtoul(market_open,&saveptr,10) && TM->tm_min<atoi(saveptr+1,NULL,10))) {
