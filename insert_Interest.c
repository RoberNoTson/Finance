 ...
 * compile: gcc -Wall -O2 -ffast-math insert_Interest.c -o insert_Interest `mysql_config --include --libs`
 */
...
int	main (int argc, char *argv[]) {
  ...
  printf("Interest pay date [%s]: ",qDate);
...
//  if (value < 0) value *= -1;
  printf("Verify this:  Add $%.2f Interest on %s\n",value,div_date);
  ...
  sprintf(query,"insert into beancounter.cash () values(\"ML-CMA\",\"%.2f\",DEFAULT,\"Interest\",DEFAULT,DEFAULT,\"%s\")",
	  value,div_date);
  if (DEBUG) printf("%s\n",query);
  if (!DEBUG) if(mysql_query(mysql,query)) print_error(mysql,"Failed to update database");
  
  puts("Database updated with new Interest cash value");
